...
if($_SERVER["REQUEST_METHOD"] === "POST"){
    ...
    if(empty($id_ci_err) && empty($password_err)){
        ...
        $sql = "SELECT id_ci,nombre, password, cargo FROM usuario WHERE id_ci = ?";
        ...
        if($stmt = mysqli_prepare($conexion, $sql)){
            ...
            if(mysqli_stmt_num_rows($stmt) == 1){
                mysqli_stmt_bind_result($stmt, $id_ci,$nombre, $hashed_password, $cargo);
                //mysqli_stmt_bind_result($stmt, $id, $usuario, $email, $hashed_password);
                if(mysqli_stmt_fetch($stmt)){
                    if(password_verify($password, $hashed_password) )
                    {
                        if($cargo=='Administrador' or $cargo=='Pastor')
                        {
                            session_start();
